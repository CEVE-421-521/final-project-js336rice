...
Basic financing, just paying up front

```{julia}
finance_basic = let  #Original finance, no loan, paying up-front 
    loan = 0 
    loan_years = 0
    loan_rate = 0 #note that rate here is in whole percents
    Finance(; loan=loan, loan_years=loan_years, loan_rate=loan_rate)
end
...
function plot_many(finance_list, initial_plot)
    ...
    for finance in finance_list
        ...
        label = "$(finance.loan_rate)% loan over $(finance.loan_years) years"
        plot!(
        initial_plot,
        elevations_tried,
        npvs ./ 1000;
        ...
        vline!([min]; label="Optimal, $(finance.loan_rate)% loan", linestyle=:dash, color=line_color)
    end
end

```

Generate many financial scenarios 
...
for i in range(1, 3)
    fin = Finance(; loan=1, loan_years=5, loan_rate=3*i)
    push!(finance_list, fin)
end

#println(finance_list)
```
...
vline!([min]; label="Optimal, no loan", linestyle=:dash, color=line_color)

plot_many(finance_list, p)
display(p)
```
